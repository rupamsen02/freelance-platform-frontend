...
function Step1({ data, onChange, onNext, file, setFile, closeJoinModal }) {
  ...
  const debouncedCheckUsername = useCallback(
    debounce(async (username) => {
      try {
        const res = await axios.get(`${API_URL}/api/users/check/exists`, {
          params: { username },
        });
        setUsernameError(
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-between relative w-full">
      ...
      <form
        ...
        className="flex flex-col justify-center space-y-4 w-84 mx-auto p-4"
      >
        <h3 className="text-2xl px-2 font-bold mb-4">Create a new account</h3>

        <div className="flex flex-col w-full px-2 space-y-4 relative">
          <span className="absolute pt-2.5 md:pt-2.5 px-1">
            <FaUser />
          </span>
          <input
            name="username"
            ...
            required
            className="w-full outline-none p-2 border-b pl-8"
          />
          {usernameError && (
            <p className="text-gray-500 mx-1 text-sm mt-1">{usernameError}</p>
          )}

          <span className="absolute pt-18 md:pt-15 lg:pt-17 px-1">
            <IoMail />
          </span>
          <input
            name="email"
          ...
          <span className="absolute pt-33 md:pt-27 lg:pt-31 px-1">
            <FaLock />
          </span>
          <input
            name="password"
          ...
          <span className="absolute bottom-28 md:bottom-23 lg:bottom-26.5 px-2">
            <AiFillPicture />
          </span>

          <label className="block mb-1 text-start font-medium">
            Profile Picture
          </label>
          <input
            type="file"
...
export default function JoinOnboarding() {
  ...
  return (
    <div>
      <button onClick={handleOpen} className="btn py-2 px-6">
        Join
      </button>
            ...
            <div className="relative flex flex-col justify-center bg-white rounded-3xl w-150 md:w-220 h-140 md:h-160 max-w-100 md:max-w-[760px] lg:max-w-[900px] overflow-hidden shadow-2xl">
              ...
              {step === 1 && (
                <Step1
                  ...
                  file={file}
                  setFile={setFile}
                  closeJoinModal={() => setOpenModal(false)}
                />
              )}
              {step === 2 && (
                <Step2
